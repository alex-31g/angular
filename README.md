...
## Реактивное программирование RxJS [07. rxjs]
Angular для работы с асинхронными операциями использует объект **Observable**, который является частью библиотеки **rxjs**.    
*Реактивное программирование* - программирование с использованием асинхронных потоков.     
*Асинхронный поток* - последовательность событий, которые упорядоченны по времени.         
Все может быть преобразовать в асинхронный поток - событие click, массив значений, ответ, который вернул сервер и т.д.    
Реактивное программирование заключается в том, что мы манипулируем разными асинхронными потоками - создаем, комбинируем, фильтруем.     
При работе с реактивным программированием мы используем API, которое предоставляем нам Observable объект.      

Создание Observable-объекта и его подписчика:
```js
import { Observable } from 'rxjs';
// Cоздание Observable-объекта выполняется с помощью конструктора Observable,  
// который принимает ф-цию, которая описывает, 
// как Observable будет работать со своими наблюдателями-подписчиками (observer)
let obserObj = new Observable((observer) => {
	// С помощью методов ниже происходит взаимодействие с наблюдателями:
