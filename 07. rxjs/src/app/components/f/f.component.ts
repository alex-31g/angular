...
export class FComponent implements OnInit {
  ...
  //    RXJS методы для создания потока: from, of, interval
  ...
  ngOnInit() {
    ...
    // interval - создает Observable-поток, который с заданным интервалом выдает события и в этом событии производит инкрементацию счетчика начиная с 0
